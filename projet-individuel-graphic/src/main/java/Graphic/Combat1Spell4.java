...
public class Combat1Spell4 extends Application {

    private TextArea textArea;

    private TextField textField;
    private ImageView imageView;
    private Image Combat1Spell;
    ...
    public void start(Stage stage) throws Exception {
        ...
        Game.wizard.attack(Game.wizard, Boss.troll, Spell.diffindo);
        ...
        if (Boss.troll.getHP() > 0) {
            Boss.troll.attack(Game.wizard, Boss.troll, Game.spells.get(0));
            ...
            String newString = "Battle won, level +1,gold+ "+Boss.troll.getGold();
            Game.wizard.setGold(Game.wizard.getGold()+Boss.troll.getGold());
            dialogues = Arrays.copyOf(dialogues, dialogues.length + 1); // 扩展数组长度
            dialogues[dialogues.length - 1] = newString; // 添加新字符串到数组
        }
        ...
        imageView = new ImageView(Combat1Spell);
    ...
    private void showCombatSpell(Stage previousStage) {
        Combat1StartPage combatStartPage = new Combat1StartPage();
        try {
