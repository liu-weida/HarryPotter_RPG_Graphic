...
public class Combat8Spell4 extends Application {

    private TextArea textArea;

    private TextField textField;
    private ImageView imageView;
    private Image Combat8Spell;
    ...
    public void start(Stage stage) throws Exception {
        ...
        Game.wizard.attack(Game.wizard, Boss.voldemort, Spell.diffindo);
        ...
        if (Boss.voldemort.getHP() > 0) {
            Boss.voldemort.attack(Game.wizard, Boss.voldemort, Game.spells.get(0));
            ...
            String newString = "Battle won, level +1,gold+ "+Boss.voldemort.getGold();
            Game.wizard.setGold(Game.wizard.getGold()+Boss.voldemort.getGold());
            dialogues = Arrays.copyOf(dialogues, dialogues.length + 1); // 扩展数组长度
            dialogues[dialogues.length - 1] = newString; // 添加新字符串到数组
        }
        ...
        imageView = new ImageView(Combat8Spell);
    ...
    private void showBattleVictory(Stage previousStage) {
        BattleVictory72 battleVictory = new BattleVictory72();
        try {
    ...
    private void showCombatSpell(Stage previousStage) {
        Combat8StartPage combatStartPage = new Combat8StartPage();
        try {
