...
public class Chapter24 extends Application {
    ...
    private String[] dialogues = {
            "Suddenly, a loud roar came from the doctor.",
            "It was a giant snake coiled in the depths of the chamber.",
            "It was spitting its tongue and seemed to have made you its dinner for the day.",
            "You look around and realize that there is no way back.",
            "There is no way out, you must fight it.",
    };
    ...
    public void start(Stage stage) throws Exception {
        ...
        // 加载默认图像
        InputStream input1 = getClass().getResourceAsStream("/image/Chapter24.jpg");
...
//        scene.setCursor(new ImageCursor(cursorImage));
    ...
    private void showNextScene() {
        if ("Gryffindor".equals(Game.wizard.getHouse().toString())) {
            Chapter25 chapter25 = new Chapter25();
            try {
                chapter25.start(mainStage);
            } catch (Exception e) {
                e.printStackTrace();
                // 如果需要，您可以在此处添加其他异常处理逻辑
            }
        } else {
            Boss.basilic.setHP(300);
            Combat2StartPage chapter02 = new Combat2StartPage();
            try {
                chapter02.start(mainStage);
            } catch (Exception e) {
                e.printStackTrace();
                // 如果需要，您可以在此处添加其他异常处理逻辑
            }
