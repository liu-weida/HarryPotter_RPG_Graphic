...
public class Chapter42 extends Application {
    ...
    private String[] dialogues = {
            "Suddenly one day, you received a distress message about a friend, and without the slightest doubt, you followed the instructions in the message to the cemetery.",
            "But there is no friend of yours here, you were deceived by Voldemort.",
            "You had to find a way to get around Voldemort and Peter Pettigrew while you were at it.",
            "Suddenly, you see the Portkey around Peter Pettigrew's waist, and through the knowledge you learned in your museum class, you know that you can teleport anywhere through it, which is the only possibility for you to be saved.",
            "But Peter Pettigrew doesn't want to give you what you want. He assumes a fighting stance and prepares to start using the spell.",
            "There is no way, you had to beat him first, fortunately Voldemort just stood aside, and did not intervene.",
    };
    ...
    public void start(Stage stage) throws Exception {
        ...
        // 加载默认图像
        InputStream input1 = getClass().getResourceAsStream("/image/Chapter41.jpg");
...
//        scene.setCursor(new ImageCursor(cursorImage));
    ...
    private void showNextScene() {
        Boss.peterPettigrew.setHP(500);
        Combat4StartPage chapter42 = new Combat4StartPage();
        try {
            chapter42.start(mainStage);
        } catch (Exception e) {
            e.printStackTrace();
