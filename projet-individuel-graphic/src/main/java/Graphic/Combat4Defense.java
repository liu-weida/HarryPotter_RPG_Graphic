...
public class Combat4Defense extends Application {

    private TextArea textArea;

    private TextField textField;
    private ImageView imageView;
    private Image CombatDefense;
    ...
    public void start(Stage stage) throws Exception {
        ...
        Wizard.defend(Boss.peterPettigrew);

        if (Wizard.ifDefense){
            dialogues = new String[]{
                    "Defend successfully and no damage will be dealt this turn.",
            };
        }else {
            Enemy.gargoyle.attack(Game.wizard, Enemy.gargoyle, Game.spells.get(0));
            dialogues = new String[]{
                    "Defensive failure.",
                    "Enemy has dealt " + Enemy.injury + " points of damage to you!",
            };
        }
        ...
        CombatDefense = new Image(input1);
        ...
        imageView = new ImageView(CombatDefense);
    ...
    private void nextDialogue() {
        if (currentDialogue < dialogues.length) {
            ...
            if (Game.property[1]<=0){
                ...
                PageTransition.smoothTransition(mainStage, mainStage.getScene().getRoot(), () -> showCombat1StartPage(mainStage));
            }

        }
    }

    private void showCombat1StartPage(Stage previousStage) {
        Combat4StartPage combat1StartPage = new Combat4StartPage();
        try {
            combat1StartPage.start(new Stage());
