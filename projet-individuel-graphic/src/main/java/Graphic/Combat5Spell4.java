...
public class Combat5Spell4 extends Application {

    private TextArea textArea;

    private TextField textField;
    private ImageView imageView;
    private Image Combat5Spell;
    ...
    public void start(Stage stage) throws Exception {
        ...
        Game.wizard.attack(Game.wizard, Boss.doloresOmbrage, Spell.diffindo);
        ...
        if (Boss.doloresOmbrage.getHP() > 0) {
            Boss.doloresOmbrage.attack(Game.wizard, Boss.doloresOmbrage, Game.spells.get(0));
            ...
            String newString = "Battle won, level +1,gold+ "+Boss.doloresOmbrage.getGold();
            Game.wizard.setGold(Game.wizard.getGold()+Boss.doloresOmbrage.getGold());
            dialogues = Arrays.copyOf(dialogues, dialogues.length + 1); // 扩展数组长度
            dialogues[dialogues.length - 1] = newString; // 添加新字符串到数组
        }
        ...
        imageView = new ImageView(Combat5Spell);
    ...
    private void showBattleVictory(Stage previousStage) {
        BattleVictory5 battleVictory = new BattleVictory5();
        try {
    ...
    private void showCombatSpell(Stage previousStage) {
        Combat5StartPage combatStartPage = new Combat5StartPage();
        try {
